...
# 1. Initialize the store (using SQLite for this example)
...
Use `upsert_blocks` to create or update blocks. This method is idempotent.

```python
store.upsert_blocks([block1, block2])
```

### Atomic Parent-Child Updates

You can atomically update a parent's children list while upserting blocks. This ensures the parent's `children_ids` are updated in the same transaction.

**Appending to a parent:**

```python
# Adds new_block as a child of parent_block
store.upsert_blocks([new_block], parent_id=parent_block.id)
```

**Inserting at a specific position:**

```python
# Inserts new_block immediately after sibling_block_id
store.upsert_blocks(
    [new_block], 
    parent_id=parent_block.id, 
    insert_after=sibling_block_id
)
```

**Top-Level Only Behavior:**
By default (`top_level_only=True`), when you provide a `parent_id`, only the blocks in your list that don't have a `parent_id` set will be added as children of the target parent. To add *all* provided blocks as children, set `top_level_only=False`.
